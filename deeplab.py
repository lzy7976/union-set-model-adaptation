...
class ResNet101(nn.Module):
    ...
    def forward(self, x, extract_feat=False, feat_only=False, feat=None):
        _, _, h, w = x.size()
        if feat is None:
            x = self.conv1(x)
            x = self.bn1(x)
...
def Deeplab(num_classes=21, init_weights=None, restore_from=None):
    ...
    if init_weights is not None:
        ...
        print('Successfully loaded initial weights')
